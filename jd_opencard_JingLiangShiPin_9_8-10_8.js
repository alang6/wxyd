...
第一个账号助力我 其他依次助力CK1
入口复制到jd：
21.0复制整段话 https:/JTiqaQVkBiGK03 京粮食品送福利，免费领京豆#yAsFqC2oFLmnk1%萊/京東/ＡΡΡヤ
入口复制到浏览器：
https://lzdz1-isv.isvjd.com/dingzhi/dz/openCard/activity/3929789?activityId=ae2674803c574c26af90243148a98312

更新地址：https://github.com/Tsukasa007/my_script
============Quantumultx===============
[task_local]
#9.8-9.30 京粮放价 京秋放价
5 0,5,23 * * * jd_opencard_Long_long_9_8-9-30.js, tag=9.8-9.30 京粮放价 京秋放价, img-url=https://raw.githubusercontent.com/tsukasa007/icon/master/jd_opencard_Long_long_9_8-9-30.png, enabled=true

================Loon==============
[Script]
cron "5 0,5,23 * * *" script-path=jd_opencard_Long_long_9_8-9-30.js,tag=9.8-9.30 京粮放价 京秋放价

===============Surge=================
9.8-9.30 京粮放价 京秋放价 = type=cron,cronexp="5 0,5,23 * * *",wake-system=1,timeout=3600,script-path=jd_opencard_Long_long_9_8-9-30.js

============小火箭=========
9.8-9.30 京粮放价 京秋放价 = type=cron,script-path=jd_opencard_Long_long_9_8-9-30.js, cronexpr="5 0,5,23 * * *", timeout=3600, enable=true
*/
const $ = new Env('9.8-9.30 京粮放价 京秋放价');
...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=ae2674803c574c26af90243148a98312&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}`
...
function getDrawRecordHasCoupon() {
    return new Promise(resolve => {
        let body = `activityId=ae2674803c574c26af90243148a98312&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
...
function saveTask() {
    return new Promise(resolve => {
        let body = `activityId=ae2674803c574c26af90243148a98312&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=2&taskValue=2736969&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}`
...
async function drawContent() {
    return new Promise(resolve => {
        let body = `activityId=ae2674803c574c26af90243148a98312&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/openCardcommon/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/805745?activityId=ae2674803c574c26af90243148a98312&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=ae2674803c574c26af90243148a98312&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/dz/openCard/startDraw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/805745?activityId=ae2674803c574c26af90243148a98312&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=ae2674803c574c26af90243148a98312&pageUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/797702?activityId=ae2674803c574c26af90243148a98312&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `activityId=ae2674803c574c26af90243148a98312&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid ? $.shareUuid : 'fb69c983254d4fcda8c0920619bc0f12'}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
