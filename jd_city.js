...
let inviteCodes = ['RtGKzL_yQ1nyeICSHtQ70Rwuuy6DA2alQ4FEYCN0BAjxQws7CQ@RtGKzeyiFQKmKITJQYUx33wrRnSEmX6MkpJx9Y24Nu0K9ixDiA','RtGKta32I17YPfL8cL5TmpNcycywJ4j-9CAPXY3kbI2Hog9e']
!(async () => {
  if (!cookiesArr[0]) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://jd.turinglabs.net/api/v2/jd/city/read/10/`, 'timeout': 1}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
