...
入口复制到浏览器：
https://lzdz1-isv.isvjd.com/dingzhi/shop/league/activity/7795914?activityId=dz2109100001616201

更新地址：https://github.com/Tsukasa007/my_script
============Quantumultx===============
[task_local]
#9.15-9.21 超级会员社
5 0,10,23 * * * jd_opencard_CaoJiHuiYuanShe_9-15_9_21.js, tag=9.15-9.21 超级会员社, img-url=https://raw.githubusercontent.com/tsukasa007/icon/master/jd_opencard_CaoJiHuiYuanShe_9-15_9_21.png, enabled=true

================Loon==============
[Script]
cron "5 0,10,23 * * *" script-path=jd_opencard_CaoJiHuiYuanShe_9-15_9_21.js,tag=9.15-9.21 超级会员社

===============Surge=================
9.15-9.21 超级会员社 = type=cron,cronexp="5 0,10,23 * * *",wake-system=1,timeout=3600,script-path=jd_opencard_CaoJiHuiYuanShe_9-15_9_21.js

============小火箭=========
9.15-9.21 超级会员社 = type=cron,script-path=jd_opencard_CaoJiHuiYuanShe_9-15_9_21.js, cronexpr="5 0,10,23 * * *", timeout=3600, enable=true
*/
const $ = new Env('9.15-9.21 超级会员社');
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            if (!checkOpenCardData.allOpenCard) {
                for (let cardListElement of checkOpenCardData.cardList) {
...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}`
        $.post(taskPostUrl('/dingzhi/shop/league/followShop', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/followShop'), async (err, resp, data) => {
            try {
                if (err) {
...
function getDrawRecordHasCoupon() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
...
function saveTask(taskType,taskValue) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=${taskType}&taskValue=${taskValue}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}`
        $.post(taskPostUrl('/dingzhi/shop/league/saveTask', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/saveTask'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function drawContent() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/common/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz2109100001616201&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/shop/league/startDraw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/805745?activityId=dz2109100001616201&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=dz2109100001616201&pageUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity/797702?activityId=dz2109100001616201&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `activityId=dz2109100001616201&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
                'Referer': 'https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activityContent',
                'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
            }
        }, async (err, resp, data) => {
            try {
...
function checkOpenCard() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100001616201&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '5d480d001b48486b908156db0bbfb203'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/shop/league/checkOpenCard', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/checkOpenCard'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
