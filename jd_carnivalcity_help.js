...
!(async () => {
  ...
  $.temp = [];
  $.updatePkActivityIdRes = await getAuthorShareCode('http://adguard.ipq.co/jd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'http://adguard.ipq.co/jd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('http://adguard.ipq.co/jd_cityShareCodes.json')
  }
  await requireConfig();
