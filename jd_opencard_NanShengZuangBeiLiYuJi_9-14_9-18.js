...
入口复制到jd：
入口复制到浏览器：
https://lzdz1-isv.isvjd.com/dingzhi/muying/recruiting/activity/6416617?activityId=74c66a86bc444402be5e2129b13b35d2

更新地址：https://github.com/Tsukasa007/my_script
============Quantumultx===============
[task_local]
#9.14-9.18 男神装备礼遇季
5 0,5,23 * * * jd_opencard_NanShengZuangBeiLiYuJi_9-14_9-18.js, tag=9.14-9.18 男神装备礼遇季, img-url=https://raw.githubusercontent.com/tsukasa007/icon/master/jd_opencard_NanShengZuangBeiLiYuJi_9-14_9-18.png, enabled=true

================Loon==============
[Script]
cron "5 0,5,23 * * *" script-path=jd_opencard_NanShengZuangBeiLiYuJi_9-14_9-18.js,tag=9.14-9.18 男神装备礼遇季

===============Surge=================
9.14-9.18 男神装备礼遇季 = type=cron,cronexp="5 0,5,23 * * *",wake-system=1,timeout=3600,script-path=jd_opencard_NanShengZuangBeiLiYuJi_9-14_9-18.js

============小火箭=========
9.14-9.18 男神装备礼遇季 = type=cron,script-path=jd_opencard_NanShengZuangBeiLiYuJi_9-14_9-18.js, cronexpr="5 0,5,23 * * *", timeout=3600, enable=true
*/
...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}`
        $.post(taskPostUrl('/dingzhi/sep2021/maleGodDay/followShop', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/followShop'), async (err, resp, data) => {
            try {
                if (err) {
...
function getPrize() {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/sep2021/maleGodDay/getPrize', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/getDrawRecordHasCoupon'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data)
                    $.log(`================== 你邀请了： ${data.data.score}`)
                }
            } catch (e) {
                $.logErr(e, resp)
            } finally {
...
function saveTask(type,value) {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=${type}&taskValue=${value}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '"546cd6561a5748cc8c0d03df32b9d66e"'}`
        $.post(taskPostUrl('/dingzhi/sep2021/maleGodDay/saveTask', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/saveTask'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function drawContent() {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/common/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/activity/805745?activityId=74c66a86bc444402be5e2129b13b35d2&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/sep2021/maleGodDay/startDraw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/activity/805745?activityId=74c66a86bc444402be5e2129b13b35d2&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            url: `https://lzdz1-isv.isvjd.com/dingzhi/muying/recruiting/activity/6416617?activityId=74c66a86bc444402be5e2129b13b35d2&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=74c66a86bc444402be5e2129b13b35d2&pageUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/activity/797702?activityId=74c66a86bc444402be5e2129b13b35d2&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            url: `https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/activityContent`,
            body: `activityId=74c66a86bc444402be5e2129b13b35d2&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
                'Referer': 'https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/activityContent',
                'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
            }
        }, async (err, resp, data) => {
            try {
...
function checkOpenCard() {
    return new Promise(resolve => {
        let body = `activityId=74c66a86bc444402be5e2129b13b35d2&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/sep2021/maleGodDay/checkOpenCard', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/sep2021/maleGodDay/checkOpenCard'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function taskPostUrl(url, body, referer) {
    return {
        ...
        headers: {
            ...
            "Referer": referer ? referer : `https://lzdz1-isv.isvjcloud.com/lzclient/dz/2021jan/eliminateGame/0713eliminate/?activityId=735c30216dc640638ceb6e63ff6d8b17&shareUuid=${$.shareUuid ? $.shareUuid : '546cd6561a5748cc8c0d03df32b9d66e'}&adsource=&shareuserid4minipg=u%2FcWHIy7%2Fx3Ij%20HjfbnnePkaL5GGqMTUc8u%2Fotw2E%20a7Ak3lgFoFQlZmf45w8Jzw&shopid=0&lng=114.062541&lat=29.541254&sid=57b59835c68ed8959d124d644f61c58w&un_area=4_48201_54794_0`,
            "User-Agent": UA,
