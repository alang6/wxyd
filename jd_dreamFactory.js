...
const inviteCodes = [
  'LyU3MkgQ9TQR4aqK0HVj5Q==@X19r6jziMAzahz96guTouA==',
  'LyU3MkgQ9TQR4aqK0HVj5Q==@X19r6jziMAzahz96guTouA=='
];
...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('http://adguard.ipq.co/jd_updateFactoryTuanId.json')
  if (!res) {
    $.http.get({url: 'http://adguard.ipq.co/jd_updateFactoryTuanId.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await updateTuanIdsCDN('http://adguard.ipq.co/jd_updateFactoryTuanId.json');
  }
